...
const JobRequestsPanel = () => {
    ...
    const filterLabels = {
        pending_approval: { label: "Pending Review", icon: <Clock size={12} />, color: "text-amber-600" },
        approved: { label: "Approved & Live", icon: <CheckCircle size={12} />, color: "text-emerald-600" },
    ...
    return (
        <div className="space-y-6">
            ...
            <AnimatePresence>
                {statusMsg && (
                    <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0 }}
                        className="px-5 py-3 rounded-2xl bg-white border border-black/10 text-gray-800 text-sm font-bold shadow-sm"
                    >
                        {statusMsg}
                    </motion.div>
                )}
            ...
            <div className="grid grid-cols-3 gap-4">
                {[
                    { label: "Pending Review", status: 'pending_approval', count: pendingJobs.length, color: "text-amber-600", bg: "bg-amber-500/5 border-amber-500/10", activeBg: "border-amber-500/30 bg-amber-500/10 shadow-md" },
                    { label: "Approved & Live", status: 'approved', count: jobs.filter(j => j.status === 'approved').length, color: "text-emerald-600", bg: "bg-emerald-500/5 border-emerald-500/10", activeBg: "border-emerald-500/30 bg-emerald-500/10 shadow-md" },
                    { label: "Rejected", status: 'rejected', count: jobs.filter(j => j.status === 'rejected').length, color: "text-red-600", bg: "bg-red-500/5 border-red-500/10", activeBg: "border-red-500/30 bg-red-500/10 shadow-md" },
                ].map(stat => (
                    <button
                        key={stat.status}
    ...
    return (
        <div className="admin-content min-h-screen overflow-x-hidden bg-[#f7f4ee] font-sans text-gray-900">
            <main className="mx-auto max-w-[1440px] px-4 pb-20 pt-8 md:px-10 md:pt-10">
                {/* ── Header ─────────────────────────────────────────────── */}
                <header className="mb-8 flex flex-col justify-between gap-8 overflow-hidden rounded-[2.5rem] border border-black/10 bg-gradient-to-br from-white via-[#fcfaf6] to-[#f4efe6] px-8 py-8 shadow-sm md:flex-row md:items-center">
                    <div>
                        <div className="mb-3 flex items-center gap-3">
                            <div className="flex h-11 w-11 items-center justify-center rounded-[1.25rem] bg-black text-white shadow-md">
                                <Layers size={18} />
                            </div>
